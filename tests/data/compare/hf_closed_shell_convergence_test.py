"""HF closed-shell sweep convergence: max error vs finest reference (energy and eigenvalues).

Reads ``fe*_R*__closed_shell.json`` under ``summary/all_electron/hf/<sweep>/`` (17 neutral
closed-shell atoms, Z in {2, 4, 10, 12, 18, 20, 30, 36, 38, 46, 48, 54, 56, 70, 80, 86, 88}).
Reference for each sweep is the finest mesh case (largest :math:`R_{max}` or :math:`N_{fe}`).

No Z exclusions are applied in the aggregate (unlike the full Z=1--92 HF set).

Run::

    python atomSFE/tests/data/compare/hf_closed_shell_convergence_test.py
    python atomSFE/tests/data/compare/hf_closed_shell_convergence_test.py --out path/to/figure.pdf
    python atomSFE/tests/data/compare/hf_closed_shell_convergence_test.py --highlight-z 10
"""
...
from summary_naming import mesh_tag_from_summary_path

_SUMMARY_DIR = _DATA_DIR / "summary"
_DEFAULT_HF_ROOT = _SUMMARY_DIR / "all_electron" / "hf"
_COMPARE_DIR = Path(__file__).resolve().parent
_CLOSED_SHELL_GLOB = "fe*_R*__closed_shell.json"
_DEFAULT_OUT_PDF = _COMPARE_DIR / "hf_closed_shell_convergence_test_summary.pdf"
_DEFAULT_OUT_HIGHLIGHT_PDF = _COMPARE_DIR / "hf_closed_shell_convergence_test_highlight.pdf"
...
def _glob_closed_shell_summaries(sweep_dir: Path) -> list[Path]:
    files = sorted(sweep_dir.glob(_CLOSED_SHELL_GLOB))
    if not files:
        raise RuntimeError(
            f"No {_CLOSED_SHELL_GLOB} in {sweep_dir} "
            "(expected filtered closed-shell HF summaries)."
        )
    return files
...
def _build_curve(
    ...
    sweep_dir = hf_root / mode
    files = _glob_closed_shell_summaries(sweep_dir)
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "HF closed-shell sweep convergence (max error vs finest case over 17 closed-shell Z)."
        ),
    )
    ap.add_argument(
        "--hf-root",
        type=Path,
        default=_DEFAULT_HF_ROOT,
        help="HF summary root (domain_radius_sweep/, finite_element_sweep/ with __closed_shell.json).",
    )
    ap.add_argument(
        "--out",
        type=Path,
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="If set, write a second figure for this closed-shell Z only (e.g. 10 for Ne).",
    )
    ap.add_argument(
        "--exclude-fe-x",
        type=float,
        nargs="*",
        default=(),
        help="Optional N_fe abscissa values to drop from the finite-element panel.",
    )
    args = ap.parse_args()
    root = args.hf_root.resolve()
    ...
    plt.close(fig)
    print(f"Wrote figure: {out_pdf}  (closed-shell subset, 17 Z)")
    ...
    if args.highlight_z is not None:
        ...
        if x_r_z.size == 0 and x_fe_z.size == 0:
            print(
                f"Warning: no data for Z={z} in closed-shell summaries under {root}; "
                "skipped highlight figure.",
            )
        else:
