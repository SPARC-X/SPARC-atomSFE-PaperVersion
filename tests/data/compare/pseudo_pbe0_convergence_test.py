"""Pseudo PBE0 sweep convergence test: max error vs finest reference.

Reads flat ``fe*_R*__*.json`` under ``summary/pseudo_potential/pbe0/<sweep>/``. Reference
...
Run::
    ...
    python atomSFE/tests/data/compare/pseudo_pbe0_convergence_test.py
    python atomSFE/tests/data/compare/pseudo_pbe0_convergence_test.py --exclude-fe-x 13
"""
...
_DEFAULT_PBE0_ROOT = _SUMMARY_DIR / "pseudo_potential" / "pbe0"
_DEFAULT_OUT_PDF = _COMPARE_DIR / "pseudo_pbe0_convergence_test_summary.pdf"
_DEFAULT_OUT_HIGHLIGHT_PDF = _COMPARE_DIR / "pseudo_pbe0_convergence_test_highlight.pdf"
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description=(
            "Pseudo PBE0 sweep convergence (max error vs finest case; Z=25,51 omitted from "
    ...
    ap.add_argument(
        "--pbe0-root",
        type=Path,
        default=_DEFAULT_PBE0_ROOT,
        help="Path to pseudo_potential/pbe0 summary root.",
    )
    ap.add_argument(
        "--out",
        type=Path,
    ...
    extra_z = frozenset()  # pseudo PBE0 aggregate skip: Z=25,51 only
